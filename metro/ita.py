...
def make_details_df(paths_list, es, base_cost):
...
def ITA_iteration(g, od, p, scale, details, flow_dict, es, paths_list, base_cost, a, b):            
    ...
    for o in od:
        ...
        if len(ds) > 0:
            ...
            if details:
                update_paths_list(paths_list, o, targets, od, paths, p, scale)
...
def ITA(g, od, base_cost = 'free_flow_time_m', P = [0.4, 0.3, 0.2, 0.1], a = 0.15, b = 4., scale = .25, details = False):
    ...
    # Compute details
    if details:
        df = make_details_df(paths_list, es, base_cost)
        df = agg_df(df)
