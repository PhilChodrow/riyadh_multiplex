...
class multiplex:
   ...
   #  		print 'The layer'  + layer + ' is already defined. This network has not been added.'
   #  	else:
			# self.layers.append(key)
			# nx.set_node_attributes(layer_dict[key], 'layer', key)
			# nx.set_edge_attributes(layer_dict[key], 'layer', key)
			# self.G = nx.disjoint_union(self.G, layer_dict[key])
	...
	Requires that each node in each network have a 'pos' tuple of format (latitude, longitude). 
	'''	
    ...
    def spatial_join_2(self, layer1, layer2, transfer_speed, base_cost, both = True):
		'''
		Adds edges to multiplex between ALL nodes of layer1 and the nodes of layer2 spatially nearest to layer1. 
		New edges are labelled 'layer1_layer2_T' and 'layer1_layer2_T' is added to self.layers.  
		Example: spatial_join(layer1 = 'metro', layer2 = 'street')
		...
		self.layers[transfer_layer_name] = None
		
		layer1_copy=self.network.subgraph([n for n,attrdict in self.network.node.items() if attrdict['layer'] == layer1])
		layer2_copy=self.network.subgraph([n for n,attrdict in self.network.node.items() if attrdict['layer'] == layer2])
		
		for n in layer1_copy.node:
			nearest, nearest_dist = utility.find_nearest(n, layer1_copy, layer2_copy)
			self.network.add_edge(n, nearest, {'layer' : transfer_layer_name, 'dist_km' : nearest_dist, 'cost_time_m'  :  nearest_dist / transfer_speed + base_cost})
			bidirectional = ""
			if both: 
				self.network.add_edge(nearest, n, {'layer' : transfer_layer_name, 'dist_km' : nearest_dist, 'cost_time_m'  : nearest_dist / transfer_speed + base_cost}) # assumes bidirectional
    ...
    def summary(self, print_summary = False):
        ...
        '''
        print 'Not implemented.'
        layers = {  layer: ( len(self.layers[layer].nodes()), len(self.layers[layer].edges()) ) for layer in self.layers.keys() }
    ...
    def to_txt(self, directory, file_name):
		...
		'''
		utility.write_nx_nodes(self.network, directory, file_name + '_nodes.txt')
		utility.write_nx_edges(self.network, directory, file_name + '_edges.txt')
